...
def _get_tax_breakdown_without_template(sales_invoice_doc):
    ...
    tax_category, exemption_reason_code = _get_invoice_level_zatca_tax_source(sales_invoice_doc)
...
def _build_legal_monetary_total(
    ...
    if prepaid_amount > tax_inclusive_amount:
        frappe.throw(
            _(
                f"ZATCA PrepaidAmount cannot exceed TaxInclusiveAmount in {currency}. "
                f"PrepaidAmount is {prepaid_amount}, TaxInclusiveAmount is {tax_inclusive_amount}."
            )
        )
